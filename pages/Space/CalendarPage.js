...
export default function CalendarPage({ route, navigation }) {
    ...
    const getCalenderDataForDate = (year, month, date) => {
        ...
        return {
            ...
            daysText,
            title: daysText,
            fullSchedule: (parsedEvents?.[year]?.[month]?.[date] ?? []).filter(e => e.spaceId === activeSpace),
        }
    }
    ...
    const getFullCalendar = (year, month) => {
        ...
        if (date.getDay() !== 0) {
            ...
            while (date.getDay() !== 6) {
                ...
                week[index].events = (parsedEvents?.[year]?.[month]?.[week[index].date] ?? []).filter(e => e.spaceId === activeSpace).slice(0, 4).map(e => {
                    return {
                        title: e.title,
                        color: e.color,
                    }
                ...
                (lastRequest && activeSpace) ?
                    <>
                        ...
                        <Component
                            key={activeSpace}
                            dayView={dayView}
                            getCalenderDataForDate={getCalenderDataForDate}
                            gotoDateData={gotoDateData}
                            getCalenderData={getCalenderData}
                            handleMonthCellPress={handleMonthCellPress}
